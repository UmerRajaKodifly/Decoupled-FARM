...
class PanoramaReconstructionOptions:
    ...
    gpu_index: str = "-1"
    use_gpu: bool = True
    # Bundle adjustment backend: CASPAR when available, else GPU Ceres.
    ba_backend: Literal["caspar", "ceres"] = "caspar"
    covisibility_path: Path | None = None
    covisibility_min_shared_points: int = 1
    show_progress: bool = True


def _supports_caspar_ba() -> bool:
    """True when this pycolmap/COLMAP build can run CASPAR BA."""
    if not hasattr(pycolmap, "BundleAdjustmentBackend"):
        return False
    if not hasattr(pycolmap.BundleAdjustmentBackend, "CASPAR"):
        return False
    # Binary probe (host or docker COLMAP).
    try:
        import shutil

        colmap = shutil.which("colmap")
        if colmap is None:
            # Enum present but no CLI — still try CASPAR via pycolmap.
            return True
        data = Path(colmap).read_bytes()
        return b"CasparBundleAdjuster" in data
    except OSError:
        return True


def _resolve_ba_backend(requested: str) -> Literal["caspar", "ceres"]:
    if requested == "ceres":
        return "ceres"
    if _supports_caspar_ba():
        return "caspar"
    logging.warning(
        "CASPAR BA requested but unavailable in this COLMAP build; "
        "falling back to GPU Ceres."
    )
    return "ceres"


def _configure_incremental_ba(
    opts: "pycolmap.IncrementalPipelineOptions",
    *,
    use_gpu: bool,
    gpu_index: str,
    ba_backend: Literal["caspar", "ceres"],
) -> None:
    """GPU Ceres local BA; CASPAR (or GPU Ceres) for global BA."""
    backend = _resolve_ba_backend(ba_backend)
    if hasattr(opts, "ba_use_gpu"):
        opts.ba_use_gpu = bool(use_gpu)
    if hasattr(opts, "ba_gpu_index"):
        opts.ba_gpu_index = str(gpu_index)

    local = pycolmap.BundleAdjustmentBackend.CERES
    global_ba = (
        pycolmap.BundleAdjustmentBackend.CASPAR
        if backend == "caspar"
        else pycolmap.BundleAdjustmentBackend.CERES
    )
    if hasattr(opts, "ba_local_backend"):
        opts.ba_local_backend = local
    if hasattr(opts, "ba_global_backend"):
        opts.ba_global_backend = global_ba
    logging.info(
        f"Incremental BA: local=CERES(GPU={use_gpu}), global={backend.upper()} "
        f"(gpu_index={gpu_index})"
    )


def _configure_global_ba(
    ...
    *,
    ...
    """Enable GPU global positioning + CASPAR/Ceres bundle adjustment."""
    backend = _resolve_ba_backend(ba_backend)
    mapper = pipeline_opts.mapper
    if hasattr(mapper, "global_positioning"):
        gp = mapper.global_positioning
        if hasattr(gp, "use_gpu"):
            gp.use_gpu = bool(use_gpu)
        if hasattr(gp, "gpu_index"):
            gp.gpu_index = str(gpu_index)
        if hasattr(gp, "min_num_images_gpu_solver"):
            gp.min_num_images_gpu_solver = 1

    ba = mapper.bundle_adjustment
    if backend == "caspar" and hasattr(ba, "caspar"):
        ba.backend = pycolmap.BundleAdjustmentBackend.CASPAR
        ba.caspar.gpu_index = str(gpu_index)
    elif hasattr(ba, "ceres"):
        ba.backend = pycolmap.BundleAdjustmentBackend.CERES
        ba.ceres.use_gpu = bool(use_gpu)
        ba.ceres.gpu_index = str(gpu_index)
    logging.info(
        f"Global BA: backend={backend.upper()} (GPU={use_gpu}, gpu_index={gpu_index})"
    )
...
def create_virtual_camera(
    *,
    ...
    Uses PINHOLE (fx=fy) so CASPAR BA can run; SIMPLE_PINHOLE is not CASPAR-supported.
    """
...
class PanoProcessor:
    ...
    def process(self, pano_name: str) -> None:
        ...
        for cam_idx, cam_from_pano_r in enumerate(self.cams_from_pano_rotation):
            ...
            # pathlib.mkdir(parents=True) is not race-safe across threads; serialize.
            with self._lock:
...
def run_perspective(
    ...
    # Pre-create per-face folders so worker threads do not race on mkdir.
    render_opts = PANO_RENDER_OPTIONS[options.render_type]
    n_faces = render_opts.num_steps_yaw * len(render_opts.pitches_deg)
    for i in range(n_faces):
        (image_dir / f"pano_camera{i}").mkdir(exist_ok=True, parents=True)
        (mask_dir / f"pano_camera{i}").mkdir(exist_ok=True, parents=True)
    ...
    if options.mapper == Mapper.INCREMENTAL:
        ...
        if hasattr(global_opts.mapper.bundle_adjustment, "refine_sensor_from_rig"):
            global_opts.mapper.bundle_adjustment.refine_sensor_from_rig = False
        _configure_global_ba(
            global_opts,
            use_gpu=options.use_gpu,
            gpu_index=options.gpu_index,
    ...
    # Optional: convert face rig -> native EQUIRECTANGULAR model.
    # Skipped when CameraModelId.EQUIRECTANGULAR is unavailable (some pycolmap
    # builds). The perspective sparse/ model is what the DA3 depth pipeline uses.
    if hasattr(pycolmap.CameraModelId, "EQUIRECTANGULAR"):
        ...
        logging.warning(
            "Skipping equirectangular conversion: "
            "pycolmap.CameraModelId.EQUIRECTANGULAR not available in this build. "
            "Perspective sparse model under sparse/ is intact."
        )
    return recs


def _cleanup_sqlite_files(database_path: Path) -> None:
    """Remove SQLite main DB and WAL/SHM/journal sidecars."""
    for suffix in ("", "-wal", "-shm", "-journal"):
        p = Path(str(database_path) + suffix) if suffix else database_path
        if p.is_file():
            p.unlink()
...
def reconstruct(
    ...
    # Stale WAL/SHM on a Docker bind mount often triggers SQLite "disk I/O error".
    # Keep the live DB on local disk (container /tmp), copy into output when done.
    out_db = output_path / "database.db"
    _cleanup_sqlite_files(out_db)

    work_dir = Path(tempfile.mkdtemp(prefix="colmap_sfm_"))
    database_path = work_dir / "database.db"
    logging.info(f"Using local SQLite database at {database_path}")
    ...
    try:
        if options.render_type == PanoRenderType.SPHERICAL:
            recs = run_spherical(
                input_image_path, options, database_path, rec_path
            )
        else:
            recs = run_perspective(
                input_image_path,
                output_path,
                options,
                database_path,
        ...
        if database_path.is_file():
            # Checkpoint WAL into the main file then copy out.
            try:
                import sqlite3
                ...
                logging.warning(f"WAL checkpoint before copy failed: {exc}")
            shutil.copy2(database_path, out_db)
        return recs
    finally:

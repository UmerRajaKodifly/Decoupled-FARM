...
Prerequisites: an NVIDIA GPU with CUDA 12.8+ drivers, Docker with the NVIDIA
container runtime, and `docker compose`. All runtime work happens inside the
container — there is no supported host-side `pip install` path; this keeps the
CUDA/ROS 2/PyTorch/YOLOE version matrix reproducible.
...
./bootstrap_models.sh      # YOLOE + MobileCLIP + SigLIP2 weights (public URLs, no account needed)
./run.sh build             # builds scene_graph:latest (CUDA 12.8 + ROS 2 Humble)
...
without it. The DINOv3 merge backbone ships in-repo (`models/dinov3-vits16`),
so nothing else is required to run.
...
<details>
<summary><b>Optional: paper-grade DINOv3 backbone (gated)</b></summary>

The pipeline auto-prefers the gated **ViT-S+/16** (`dinov3-vits16plus`) — the
...
[`EVALUATION.md`](EVALUATION.md) for measured impact. To enable the paper
backbone, accept the license at
[facebook/dinov3-vits16plus-pretrain-lvd1689m](https://huggingface.co/facebook/dinov3-vits16plus-pretrain-lvd1689m), then:

```bash
huggingface-cli download facebook/dinov3-vits16plus-pretrain-lvd1689m --local-dir models/dinov3-vits16plus
```

Both variants are `hidden_size=384`, so nothing else changes — it is picked up
...
├── models/                    # dinov3-vits16 (committed); bootstrap_models.sh fetches the rest

...
DISABLE_INFERENCE_MODE = os.environ.get("ULTRALYTICS_DISABLE_INFERENCE_MODE", "").strip().lower() in {
    "1",
    "true",
    "yes",
}
...
def smart_inference_mode():
    """Wrap functions with torch.inference_mode() unless explicitly disabled."""

    def decorate(fn):
        """Applies the desired torch decorator at definition time."""
        if TORCH_1_9 and torch.is_inference_mode_enabled():
            return fn  # already inside inference_mode context
        if DISABLE_INFERENCE_MODE or not TORCH_1_9:

...
Pakistan **calls HK only** for Phase 4b caption + Phase 4c embed via [`vlm_client.py`](../phase4-visual-query/vlm_client.py) (`VlmClient`).
...
[`phase4-visual-query/vlm_client.py`](../phase4-visual-query/vlm_client.py) implements:
...
Backend: set `VLLM_*` env vars in `Decoupled-FARM/.env` (see §4).
